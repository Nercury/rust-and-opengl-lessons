...
use resources::{self, Resources, Resource};
use na;
...
impl Program {
    ...
    pub fn set_uniform_matrix_4fv(&self, location: i32, value: &na::Matrix4<f32>) {
        unsafe {
            self.gl.UniformMatrix4fv(location,
                                     1,
                                     gl::FALSE,
...
impl Shader {
    ...
    pub fn from_resource(
        ...
        Ok(Shader { resource, gl: gl.clone(), id })
    }
...
fn shader_from_source(
    ...
    let mut source = resource.get()
        .map_err(|e| Error::ResourceLoad { name: resource.name(), inner: e })?;
    ...
    if success == 0 {
        ...
        return Err(Error::CompileError { name: resource.name(), message: error.to_string_lossy().into_owned() });
    }
